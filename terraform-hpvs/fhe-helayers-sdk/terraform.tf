...
# the VPC
resource "ibm_is_vpc" "fhe_hpvs_vpc" {
  name = format("%s-vpc", var.prefix)
  tags = local.tags
}

# the security group
resource "ibm_is_security_group" "fhe_hpvs_security_group" {
  name = format("%s-security-group", var.prefix)
  vpc  = ibm_is_vpc.fhe_hpvs_vpc.id
  tags = local.tags
}
...
resource "ibm_is_security_group_rule" "fhe_hpvs_outbound" {
  group     = ibm_is_security_group.fhe_hpvs_security_group.id
  direction = "outbound"
  remote    = "0.0.0.0/0"
}

# rule that allows the VSI to accept inbound connections, this is required
resource "ibm_is_security_group_rule" "fhe_hpvs_inbound" {
  group     = ibm_is_security_group.fhe_hpvs_security_group.id
  direction = "inbound"
  remote    = "0.0.0.0/0"
}
...
# the subnet
resource "ibm_is_subnet" "fhe_hpvs_subnet" {
...
# an outbound rule
resource "ibm_is_public_gateway" "fhe_hpvs_gateway" {
...
resource "ibm_is_subnet_public_gateway_attachment" "fhe_hpvs_gateway_attachment" {
  subnet         = ibm_is_subnet.fhe_hpvs_subnet.id
  public_gateway = ibm_is_public_gateway.fhe_hpvs_gateway.id
}
...
# it can be random
resource "tls_private_key" "fhe_hpvs_rsa_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

# we only need this because VPC expects this
resource "ibm_is_ssh_key" "fhe_hpvs_sshkey" {
  name       = format("%s-key", var.prefix)
  public_key = tls_private_key.fhe_hpvs_rsa_key.public_key_openssh
  tags       = local.tags
}
...
# construct the VSI
resource "ibm_is_instance" "fhe_hpvs_vsi" {
  name    = format("%s-vsi", var.prefix)
  image   = data.hpcr_image.hyper_protect_image.image
  profile = var.profile
  keys    = [ibm_is_ssh_key.fhe_hpvs_sshkey.id]
  vpc     = ibm_is_vpc.fhe_hpvs_vpc.id
  tags    = local.tags
  zone    = "${var.region}-${var.zone}"
  ...
  primary_network_interface {
    name            = "eth0"
    subnet          = ibm_is_subnet.fhe_hpvs_subnet.id
    security_groups = [ibm_is_security_group.fhe_hpvs_security_group.id]
  }
}

resource "ibm_is_floating_ip" "fhe_hpvs_vsi_floatingip" {
  name   = ibm_is_instance.fhe_hpvs_vsi.name
  target = ibm_is_instance.fhe_hpvs_vsi.primary_network_interface[0].id
}

output "hpvs_vsi_floating_ip" {
  value = ibm_is_floating_ip.fhe_hpvs_vsi_floatingip.address
}
