...
# the VPC
resource "ibm_is_vpc" "dynamic_registry_vpc" {
  name = format("%s-vpc", var.prefix)
  tags = local.tags
}

# the security group
resource "ibm_is_security_group" "dynamic_registry_security_group" {
...
resource "ibm_is_security_group_rule" "dynamic_registry_outbound" {
  group     = ibm_is_security_group.dynamic_registry_security_group.id
  direction = "outbound"
  remote    = "0.0.0.0/0"
}

# the subnet
resource "ibm_is_subnet" "dynamic_registry_subnet" {
...
# an outbound rule
resource "ibm_is_public_gateway" "dynamic_registry_gateway" {
...
resource "ibm_is_subnet_public_gateway_attachment" "dynamic_registry_gateway_attachment" {
  subnet         = ibm_is_subnet.dynamic_registry_subnet.id
  public_gateway = ibm_is_public_gateway.dynamic_registry_gateway.id
}
...
# it can be random
resource "tls_private_key" "dynamic_registry_rsa_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

# we only need this because VPC expects this
resource "ibm_is_ssh_key" "dynamic_registry_sshkey" {
  name       = format("%s-key", var.prefix)
  public_key = tls_private_key.dynamic_registry_rsa_key.public_key_openssh
  tags       = local.tags
}
...
resource "ibm_is_instance" "dynamic_registry_vsi" {
  ...
  primary_network_interface {
    name            = "eth0"
    subnet          = ibm_is_subnet.dynamic_registry_subnet.id
    security_groups = [ibm_is_security_group.dynamic_registry_security_group.id]
  }
